 ...
 * - changed i2c_init to return 1 iff both SDA and SCL are high
...
unsigned char __attribute__ ((noinline)) i2c_init(void) __attribute__ ((used));

// Start transfer function: <addr> is the 8-bit I2C address (including the R/W
// bit). 
// Return: 1 if the slave replies with an "acknowledge", false otherwise
unsigned char __attribute__ ((noinline)) i2c_start(uint8_t addr) __attribute__ ((used)); 
...
// stop condition.
// Return: 1 if the slave replies with an "acknowledge", false otherwise
unsigned char __attribute__ ((noinline)) i2c_rep_start(uint8_t addr) __attribute__ ((used));
...
// Return: 1 if the slave replies with an "acknowledge", false otherwise
unsigned char __attribute__ ((noinline)) i2c_write(uint8_t value) asm("ass_i2c_write") __attribute__ ((used));


// Read one byte. If <last> is 1, we send a NAK after having received 
// the byte in order to terminate the read sequence. 
uint8_t __attribute__ ((noinline)) i2c_read(unsigned char last) __attribute__ ((used));
...
unsigned char i2c_init(void)
{
  __asm__ __volatile__ 
    (" cbi      %[SDADDR],%[SDAPIN]     ;release SDA \n\t" 
     ...
     " ret                              ;if low return with false \n\t" 
     " ldi      r24,1                   ;set return value to 1 \n\t"
     " ret "
     : :
...
unsigned char  i2c_start(uint8_t addr)
{
  __asm__ __volatile__ 
    (
#if I2C_NOINTERRUPT
...
unsigned char  i2c_rep_start(uint8_t addr)
{
  __asm__ __volatile__ 
...
unsigned char i2c_write(uint8_t value)
{
  __asm__ __volatile__ 
    (
     " sec                              ;set carry flag \n\t"
...
uint8_t i2c_read(unsigned char last)
{
  __asm__ __volatile__ 
    (
     " ldi  r23,0x01 \n\t"
